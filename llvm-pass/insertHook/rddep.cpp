...
namespace {
  ...
  struct RddepAnalysis : public FunctionPass {
    ...
    *************************************************************************/
    ...
    void insert_hook(Function &F, Json::Value &Root, Module &M, Function * MF){
        // for (auto const& id : PolicyRoot.getMemberNames()) {
        //     errs() << id << "\n";
        // }

        // errs() << viewSwitcher->getName();
        ...
        //                     if (callee) {
        //                         // errs() << callee->getName() << "\n"; 
        //                         if (Root.isMember(callee->getName())){
        //                             // errs() << callee->getName() << "\n";
        //                             // IRBuilder<> builder(inst);
        //                             Instruction *newInst = CallInst::Create(viewSwitcher, "");
        //                             // auto &IList = I->getParent()->getInstList();
        //                             // IList.insert(I, newInst);
        //                             // auto t = dyn_cast<Instruction>(newInst);
        //                             // t->insertBefore(I);
        //                             // auto &FirstI = *IList.begin();
        //                             newInst->insertBefore(I);
        //                             // I->insertBefore(newInst);
        //                             // newInst->dump();
        //                             // newInst->insertBefore(&*itr);
        //                         }
        //                     }
        //                 }
        //         }
        ...
        Constant *hookFunc;
        hookFunc = M.getOrInsertFunction("view_switch_to_rd_and_log", Type::getVoidTy(M.getContext()), (Type*)0);

        Function *viewSwitcher;
        viewSwitcher=cast<Function>(hookFunc);
...
//                                     CallInst * III = CallInst::Create(&F,"InsttoCallInst",II);
                ...
                if(CallInst * III = dyn_cast<CallInst>(II))
                    {
                        ...
                        if(III->getCalledFunction()!=NULL&&III->getCalledFunction()->getName()=="_Z12check_latlng8Location")
                        {
                            errs() <<III->getCalledFunction()->getName()<<" function found!\n";
                            // CallInst *newInst = CallInst::Create(viewSwitcher, "");
                            // newInst->insertBefore(III);
                            // dyn_cast<BasicBlock>(BI)->getInstList().insert(&III, &newInst);
                            // Constant *hookFunc;
                            // hookFunc = M.getOrInsertFunction("view_switch_to_rd_and_log", Type::getVoidTy(M.getContext()), (Type*)0);
                            ...
                            CallInst::Create(viewSwitcher, "", III); // insert before III
    ...
    bool doInitialization(Module &M) override{
        ...
        std::vector<Type*> tramp_argus;
        tramp_argus.push_back(Type::Type::getInt32Ty(M.getContext()));
        tramp_argus.push_back(Type::Type::getInt32Ty(M.getContext()));

        FunctionType *FT = FunctionType::get(Type::getVoidTy(M.getContext()), tramp_argus, false);
        // Function * MF = Function::Create((FunctionType*)FT, Function::ExternalLinkage,"trampoline", &M);

        // FunctionType *FT = FunctionType::get(Type::getVoidTy(M.getContext()), false);
        Function * MF = Function::Create((FunctionType*)FT, Function::ExternalLinkage,"view_switch_to_rd_and_log", &M);
        ...
        // policyFile.open(RddepAnalysisResults, std::ios::in);
        ...
        // for (Function & F : M.functions()){
        //     if(F.getName().startswith("_Z") || !F.isDeclaration()){
        //         insert_hook(F,PolicyRoot,M,MF);
        //     }
        // }
